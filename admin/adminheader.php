...
if (isset($_SESSION['loginemail'])) {
  ...
  $data = $conn->query("SELECT * FROM `admin` WHERE adminemail='$adminemail'");
  if (mysqli_num_rows($data) > 0) {
    $result = mysqli_fetch_assoc($data);
    $adminname = $result['adminname'];
    $data2 = $conn->query("SELECT * FROM `uppcloth` WHERE adminemail='$adminemail'");
  ...
  header("location:../user/trandstore.php");
}
    ...
    <ul class="navbar-nav px-3">
      <li class="nav-item text-nowrap">
        <form action="" method="post">
          <button  name="logoutadmin">Sign out</button>
        </form>
      </li>
    </ul>
  </nav>
          ...
          <ul class="nav flex-column">
            <li class="nav-item">
              <a class="nav-link active" aria-current="page" href="dashboard.php">
                ...
                <?php echo  $adminemail ?>Dashboard
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="upperclothlayout.php">

                Add Upper Cloth Products
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="drescription.php">
                Add Description
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="addprice.php">
                Add Price
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="otherinfo.php">
